...
#'
#' @description
#' The `random_chisquared_walk` function generates multiple random walks in 1, 2, or 3 dimensions.
...
#' The user can control the number of walks, steps per walk, degrees of freedom (`df`), and the non-centrality parameter (`ncp`).
...
#' @param .df Degrees of freedom for the chi-squared distribution. Default is 5.
#' @param .ncp Non-centrality parameter (non-negative). Default is 0.
#' @param .initial_value A numeric value indicating the initial value of the walks. Default is 0.
#' @param .samp A logical value indicating whether to sample the chi-squared distribution values. Default is TRUE.
...
#'
#' @export
random_chisquared_walk <- function(
  .num_walks = 25, .n = 100, .df = 5, .ncp = 0,
  ...
  if (.df < 0) {
    rlang::abort(".df must be non-negative", use_cli_format = TRUE)
  }
  if (.ncp < 0) {
    rlang::abort(".ncp must be non-negative", use_cli_format = TRUE)
  }
  ...
  generate_walk <- function(walk_num) {
    rand_steps <- purrr::map(
      dim_names,
      ~ if (samp) {
        sample(stats::rchisq(n, df, ncp = ncp), size = periods, replace = replace)
      } else {
        stats::rchisq(periods, df, ncp = ncp)
      }
    )
  ...
  attr(res, "fns")           <- "random_chisquared_walk"
